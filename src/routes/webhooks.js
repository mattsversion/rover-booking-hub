...
webhooks.post('/sms-forward', async (req, res) => {
  try {
    ...
    try {
      if (!isRelay && clientId) {
        const c = await prisma.client.findUnique({ where: { id: clientId }, select: { trusted: true } });
        if (c?.trusted) {
          // Auto-confirm only those still PENDING
          await prisma.booking.updateMany({
            where: { id: { in: createdIds }, status: 'PENDING' },
            data: { status: 'CONFIRMED', notes: 'auto-confirmed: trusted client' }
          });
        }
      }
    } catch (e) {
